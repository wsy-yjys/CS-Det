...
class Model(nn.Module):
    ...
    def __init__(self, model: Union[str, Path] = 'yolov8n.pt', task=None, imgsz=640) -> None:
        """
        ...
        if suffix in ('.yaml', '.yml'):
            self._new(model, task, imgsz=imgsz)
        else:
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=True, imgsz=640):
        """
        Initializes a new model and infers the task type from the model definitions.
    ...
    def predict(self, source=None, stream=False, predictor=None, **kwargs):
        ...
        # get_raw_output
        # if 'get_raw_optout' in args:
        #     return self.predictor.predict_cli(source=source) if is_cli else self.predictor(source=source, stream=stream, get_raw_optout=True)
        # else:
    ...
    def val(self, validator=None, **kwargs):
        ...
        # --------------------------- # test speed
        if args["testspeed"]:
            validator.fps_repeat(model=self.model, repeat=13, imgsz=kwargs.get("imgsz")) # test speed
        else:
            validator(model=self.model)     # normal val
        # --------------------------- #
